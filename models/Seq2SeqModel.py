...
class Seq2SeqModel:
    def __init__(self, batcher, cell_size=256, num_layers=2):
        # Constants
    ...
    def encoding_layer(self, input):
        for i in range(self.num_layers):
            with tf.variable_scope('encoder_{}'.format(i)):
                ...
                cell_fw = rnn.DropoutWrapper(cell_fw, input_keep_prob=self.keep_prob)

                cell_bw = rnn.LSTMCell(self.cell_size, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=2))
                cell_bw = rnn.DropoutWrapper(cell_bw, input_keep_prob=self.keep_prob)
